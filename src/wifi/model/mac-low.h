...
public:
  ...
  void SetCurrentSpEnd(Time end);
  Time GetCurrentSpEnd(void) const;
...
public:
  ...
  /**
   * Set short beamforming interframe space (SBIFS) interval of this MacLow.
   *
  ...
  /**
   * Set beam refinement protocol interframe space (BRPIFS) interval of this MacLow.
   *
   * \param brpifs BRPIFS of this MacLow.
   */
  ...
  bool DmgStopAggregation (Ptr<const Packet> peekedPacket, WifiMacHeader peekedHdr, Ptr<Packet> aggregatedPacket, uint16_t size) const;
  /**
   *
  ...
  Time GetNextSimpleDataExchangeDuration (Ptr<const Packet> packet,
						const WifiMacHeader* hdr);
  Time GetNextSimpleAmpduExchangeDuration (Ptr<const Packet> packet,
						const WifiMacHeader* hdr);
  ...
  // Called by EdcaTxopN at the end of SP
  void CancelBlockAckTimeout (void);

  /* Return the Ip address of the packet*/
  std::pair <Ipv4Address,Ipv4Address> InspectIpv4Addr(Ptr<const Packet> packet);
...
private:
  ...
  Mac48Address m_macSampling;
  uint16_t m_seqSampling;

  std::map<Mac48Address, Time> m_samplingStartTime;
  std::map<Mac48Address, Time> m_sampledDuration; //!< Time used for the entire nMpdu transmission (incl. receiving Ack.)
  std::map<Mac48Address, uint16_t> m_sampleVarCounter;
