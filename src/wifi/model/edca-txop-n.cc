...
: m_manager (0),
	...
	m_nextSpStart (NanoSeconds(0)),
	m_nextSpStop (NanoSeconds(0))
{
...
EdcaTxopN::RemoveRetransmitPacket (uint8_t tid, Mac48Address recipient, uint16_t seqnumber)
{
	...
	Time
EdcaTxopN::GetNextSimpleDataExchangeDuration(Ptr<const Packet> packet,
		const WifiMacHeader* hdr)
{
	if (m_blockAckThreshold > 0) {
		return m_low->GetNextSimpleAmpduExchangeDuration(packet,
				hdr);
	} else {
		return m_low->GetNextSimpleDataExchangeDuration(packet,
				hdr);
	}
}
...
void EdcaTxopN::NotifyAccessGranted (void)
{
	...
	if (m_currentPacket == 0)
	{
		...
		if (m_isDmg && m_baManager->HasBarByAddress(m_currentBar, m_dmgNextSpDestination)) {
			SendBlockAckRequest (m_currentBar);
			return;
		} else if (m_baManager->HasBar (m_currentBar))
		{
			//if (!m_isDmg) {
			SendBlockAckRequest (m_currentBar);
			return;
		...
		if (m_isDmg) {
			m_currentPacket = m_baManager->GetNextPacketByAddress (m_currentHdr, m_dmgNextSpDestination);
		} else {
			m_currentPacket = m_baManager->GetNextPacket (m_currentHdr);
		}
		if (m_currentPacket == 0)
		{
			if (m_isDmg && (m_queue->PeekByAddress (&m_currentHdr, WifiMacHeader::ADDR1, m_dmgNextSpDestination, m_dmgNextSpSrcSink, &m_currentPacketTimestamp) == 0)) {
				NS_LOG_DEBUG ("no available packets in the queue");
				return;
			} else if (m_queue->PeekFirstAvailable (&m_currentHdr, m_currentPacketTimestamp, m_qosBlockedDestinations) == 0)
			{
				NS_LOG_DEBUG ("no available packets in the queue");
				return;
			}
			...
			if (m_isDmg) {
				m_currentPacket = m_queue->DequeueByAddress (&m_currentHdr, WifiMacHeader::ADDR1, m_dmgNextSpDestination, m_dmgNextSpSrcSink);
			} else {
		...
		else
		{
			...
			if (m_currentHdr.IsQosData ()
					&& m_queue->PeekByTidAndAddress (&peekedHdr, m_currentHdr.GetQosTid (),
						WifiMacHeader::ADDR1, m_currentHdr.GetAddr1 (), m_dmgNextSpSrcSink, &tstamp)
					&& !m_currentHdr.GetAddr1 ().IsBroadcast ()
					&& m_aggregator != 0 && !m_currentHdr.IsRetry ())
			{
				/* Here we do MSDU aggregation */
				...
				Ptr<const Packet> peekedPacket = m_queue->PeekByTidAndAddress(&peekedHdr, m_currentHdr.GetQosTid (),
						WifiMacHeader::ADDR1, 
						m_currentHdr.GetAddr1 (), m_dmgNextSpSrcSink, &tstamp);
			...
			if (m_isDmg) { //align antenna with destination
                                if(m_currentHdr.GetAddr1() != m_dmgBeaconInterval->GetNextTxSpDestination()){
				NS_LOG_DEBUG("Pkt to " << m_currentHdr.GetAddr1() << " nextTx " << m_dmgBeaconInterval->GetNextTxSpDestination() <<" m_dmgNextSpD" << m_dmgNextSpDestination);
                                        

                                }
				NS_ASSERT(m_currentHdr.GetAddr1() == m_dmgBeaconInterval->GetNextTxSpDestination());

				m_dmgAntennaController->PointAntenna(m_dmgBeaconInterval->
						GetNextTxSpDestinationMobility()->GetPosition());

				params.SetCurrentSpEnd(m_dmgBeaconInterval->GetNextTxSpStop());

				NS_LOG_DEBUG("Start Transmission to " << m_currentHdr.GetAddr1());
			}
...
EdcaTxopN::GotAck (double snr, WifiMode txMode)
{
	...
	std::map<Mac48Address, uint64_t>::iterator itSuccesses =
		m_successesPerDestination.find(m_currentHdr.GetAddr1());

	if (itSuccesses != m_successesPerDestination.end()) {
		itSuccesses->second++;
	} else {
		m_successesPerDestination.insert(
				std::pair<Mac48Address, uint64_t>(m_currentHdr.GetAddr1(), 1));
	}
...
EdcaTxopN::MissedAck (void)
{
	...
	std::map<Mac48Address, uint64_t>::iterator itFailures =
		m_failuresPerDestination.find(m_currentHdr.GetAddr1());

	if (itFailures != m_failuresPerDestination.end()) {
		itFailures->second++;
	} else {
		m_failuresPerDestination.insert(
				std::pair<Mac48Address, uint64_t>(m_currentHdr.GetAddr1(), 1));
	}
...
EdcaTxopN::MissedBlockAck (void)
{
	...
	//NS_ASSERT(!m_isDmg);
	m_manager->MissedBlockAck();
	NS_LOG_FUNCTION ("back to "<<this);
	//if (NeedBarRetransmission() && !m_isDmg)
	if (NeedBarRetransmission())
	{
		NS_LOG_DEBUG ("Need bar retransmission");
		if (!GetAmpduExist())
		{
	...
	if (m_isDmg) {
		...
		if (!m_dcf->IsAccessRequested()) {
			...
			if (m_currentPacket != 0 ) {
				...
				startAccess = true;
			} else if (m_baManager->HasBarByAddress(barTmp, m_dmgNextSpDestination)) {
				hdr.SetType (WIFI_MAC_CTL_BACKREQ);
				hdr.SetAddr1 (barTmp.recipient);
				hdr.SetAddr2 (m_low->GetAddress ());
				hdr.SetAddr3 (m_low->GetBssid ());
				hdr.SetDsNotTo ();
				hdr.SetDsNotFrom ();
			...
			} else if (packet = m_baManager->PeekNextPacketByAddress(hdr,
						m_dmgNextSpDestination, &packetTimestamp)) {
				startAccess = true;
			} else if (packet = m_queue->PeekByAddress(&hdr, WifiMacHeader::ADDR1,
						m_dmgNextSpDestination, m_dmgNextSpSrcSink, &packetTimestamp)) {
				startAccess = true;
			}

			if (startAccess) {
				NS_LOG_DEBUG("startAccess");
				m_manager->SetNextSimpleDataExchangeDuration(
						GetNextSimpleDataExchangeDuration(packet, &hdr));
				m_manager->RequestAccess (m_dcf);
			}
		}

	} else {NS_LOG_DEBUG("requestAccess");
...
EdcaTxopN::StartAccessIfNeeded (void)
{
	...
	if (m_isDmg) {
		...
		if ((m_currentPacket == 0) && !m_dcf->IsAccessRequested()) {
			WifiMacHeader hdr;
			struct Bar barTmp;
			Time packetTimestamp;
			Ptr<const Packet> packet;
			...
			if (m_baManager->HasBarByAddress(barTmp, m_dmgNextSpDestination)) {
				hdr.SetType (WIFI_MAC_CTL_BACKREQ);
				hdr.SetAddr1 (barTmp.recipient);
				hdr.SetAddr2 (m_low->GetAddress ());
				hdr.SetAddr3 (m_low->GetBssid ());
...
EdcaTxopN::NeedBarRetransmission (void)
{
	...
	if (m_currentHdr.IsQosData ())
	{
		NS_LOG_DEBUG("m_currentHdr.IsQosData ()");
		tid = m_currentHdr.GetQosTid ();

		seqNumber = m_currentHdr.GetSequenceNumber ();
		NS_LOG_DEBUG("seqNumber "<<seqNumber);
	}
	else if (m_currentHdr.IsBlockAckReq ())
	{
		NS_LOG_DEBUG("m_currentHdr.IsBlockAckReq () to "<< m_currentHdr.GetAddr1 () <<" "<<m_currentHdr.GetSize ());
		CtrlBAckRequestHeader baReqHdr;
		m_currentPacket->PeekHeader (baReqHdr);
		NS_LOG_DEBUG("tid "<<tid);
		tid = baReqHdr.GetTidInfo ();
		NS_LOG_DEBUG("tid "<<tid);
	...
	{
		NS_LOG_DEBUG("m_currentHdr.IsBlockAck ()");
		CtrlBAckResponseHeader baRespHdr;
		m_currentPacket->PeekHeader (baRespHdr);
		tid = baRespHdr.GetTidInfo ();
		NS_LOG_DEBUG("tid "<<tid);
...
EdcaTxopN::GetBlockAckThreshold (void) const
{
	...
	void
EdcaTxopN::ForceOriginatorBlockAckAgreement(Mac48Address dest, uint8_t tid,
		uint16_t timeout, bool immediateBAck)
{
	NS_LOG_DEBUG("Force Originator Block Ack Agreement with "<< dest);
	 ...
	 */
	reqHdr.SetBufferSize (1023);
	reqHdr.SetTimeout (timeout);
	/* For now we suppose this method is called before the simulations starts */
	...
	m_baManager->UpdateAgreement (&respHdr, dest);

	m_low->CreateBlockAckAgreement(&respHdr, dest, reqHdr.GetStartingSequence ());
}
...
EdcaTxopN::BaTxFailed (const WifiMacHeader &hdr)
{
	...
	uint64_t
EdcaTxopN::GetFailures (Mac48Address mac)
{
	if (m_blockAckThreshold > 0) {
		return m_baManager->GetMpduFailures(mac);
	} else {
		std::map<Mac48Address, uint64_t>::iterator itFailures =
			m_failuresPerDestination.find(mac);

		if (itFailures != m_failuresPerDestination.end()) {
			return itFailures->second;
		} else {
			return 0;
		}
	}
	...
	uint64_t
EdcaTxopN::GetSuccesses (Mac48Address mac)
{
	if (m_blockAckThreshold > 0) {
		return m_baManager->GetMpduSuccesses(mac);
	}
	else{
		std::map<Mac48Address, uint64_t>::iterator itSuccesses =
			m_successesPerDestination.find(mac);

		if (itSuccesses != m_successesPerDestination.end()) {
			return itSuccesses->second;
		}
		else {
			return 0;
		}
	...
	void
EdcaTxopN::SetDmgBeaconInterval (Ptr<DmgBeaconInterval> dmgBi)
{
	m_dmgBeaconInterval = dmgBi;
}

	Ptr<DmgBeaconInterval>
EdcaTxopN::GetDmgBeaconInterval (void)
{
	return m_dmgBeaconInterval;
}

	void
EdcaTxopN::SetDmgAntennaController (Ptr<DmgAntennaController> dmgBi)
{
	m_dmgAntennaController = dmgBi;
}

	Ptr<DmgAntennaController>
EdcaTxopN::GetDmgAntennaController (void)
{
	return m_dmgAntennaController;
}

	void
EdcaTxopN::SetIsDmg(bool isDmg)
{
	m_isDmg = isDmg;
	//m_queue->SetIsDmg(m_isDmg);
}

	bool
EdcaTxopN::IsDmg(void)
{
	return m_isDmg;
}

	void
EdcaTxopN::StartDmgSp ()
{
	NS_LOG_FUNCTION(this);
	Time now = Simulator::Now();

	m_nextSpStop = m_dmgBeaconInterval->GetNextTxSpStop();
	m_dmgSpStop = Simulator::Schedule (m_nextSpStop - now,
			&EdcaTxopN::StopDmgSp, this);

	m_dmgNextSpDestination = m_dmgBeaconInterval->GetNextTxSpDestination();
	m_dmgNextSpSrcSink = m_dmgBeaconInterval->GetNextTxSpSrcSinkIpv4Address();

	NS_LOG_DEBUG(now<<"(now). Starting an Tx SP, current SP stops at "<< m_nextSpStop);
	StartAccessIfNeeded();
}
...
EdcaTxopN::StopDmgSp ()
{
	NS_LOG_FUNCTION(this);
	Time now = Simulator::Now();
	NS_LOG_DEBUG(now<<"(now). Stopping an Tx SP, current SP started at "<< m_nextSpStart);

	m_nextSpStart = m_dmgBeaconInterval->GetNextTxSpStart();
	m_nextSpStop = m_dmgBeaconInterval->GetNextTxSpStop();

	NS_ASSERT(now <= m_nextSpStart);

	m_dmgSpStart = Simulator::Schedule (m_nextSpStart - now,
			&EdcaTxopN::StartDmgSp, this);
	...
	//m_low->CancelBlockAckTimeout ();
}

	void
EdcaTxopN::StartDmgSpTracking (void)
{
	NS_LOG_FUNCTION(this);
	Time now = Simulator::Now();
	...
	void
EdcaTxopN::SetMaxQueuePacketNumberPerDestination (uint32_t maxPacket)
{
	//m_queue->SetMaxSizePerDestination(maxPacket);
}
