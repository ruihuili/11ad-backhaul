...
TypeId CtrlBAckRequestHeader::GetTypeId(void)
{
...
TypeId CtrlBAckRequestHeader::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}
...
uint16_t CtrlBAckRequestHeader::GetBarControl(void) const
{
    NS_LOG_FUNCTION (this);
    uint16_t res = 0;
    if (m_barAckPolicy)
...
uint16_t CtrlBAckRequestHeader::GetStartingSequenceControl(void) const
{
    NS_LOG_FUNCTION (this);
...
void CtrlBAckRequestHeader::SetType(enum BlockAckType type)
{
...
bool CtrlBAckRequestHeader::MustSendHtImmediateAck(void) const
{
    NS_LOG_FUNCTION (this);
    return m_barAckPolicy;
}

uint8_t CtrlBAckRequestHeader::GetTidInfo(void) const
{
    NS_LOG_FUNCTION (this);
...
uint16_t CtrlBAckRequestHeader::GetStartingSequence(void) const
{
    NS_LOG_FUNCTION (this);
    return m_startingSeq;
}

bool CtrlBAckRequestHeader::IsBasic(void) const
{
    NS_LOG_FUNCTION (this);
    return (!m_multiTid && !m_compressed) ? true : false;
}

bool CtrlBAckRequestHeader::IsCompressed(void) const
{
    NS_LOG_FUNCTION (this);
    return (!m_multiTid && m_compressed) ? true : false;
}

bool CtrlBAckRequestHeader::IsMultiTid(void) const
{
    NS_LOG_FUNCTION (this);
    return (m_multiTid && m_compressed) ? true : false;
}
...
bool CtrlBAckResponseHeader::MustSendHtImmediateAck (void) const
{
    NS_LOG_FUNCTION (this);
    return (m_baAckPolicy) ? true : false;
}

uint8_t CtrlBAckResponseHeader::GetTidInfo (void) const
{
    NS_LOG_FUNCTION (this);
    uint8_t tid = static_cast<uint8_t> (m_tidInfo);
    return tid;
...
uint16_t CtrlBAckResponseHeader::GetStartingSequence (void) const
{
    NS_LOG_FUNCTION (this);
    return m_startingSeq;
}
...
uint8_t CtrlBAckResponseHeader::IndexInBitmap (uint16_t seq) const
{
  NS_LOG_FUNCTION (this << seq);
  uint8_t index;
...
bool CtrlBAckResponseHeader::IsInBitmap (uint16_t seq) const
{
  NS_LOG_FUNCTION (this << seq);
  return (seq - m_startingSeq + 4096) % 4096 < 64;
}

const uint16_t* CtrlBAckResponseHeader::GetBitmap (void) const
{
    NS_LOG_FUNCTION (this);
    return bitmap.m_bitmap;
}

uint64_t CtrlBAckResponseHeader::GetCompressedBitmap (void) const
{
    NS_LOG_FUNCTION (this);
    return bitmap.m_compressedBitmap;
}

void CtrlBAckResponseHeader::ResetBitmap (void)
{
    NS_LOG_FUNCTION (this);
    memset (&bitmap, 0, sizeof (bitmap));
}
...
NS_OBJECT_ENSURE_REGISTERED (CtrlDMG_Poll);

CtrlDMG_Poll::CtrlDMG_Poll()
    : m_responseOffset(0)
{
    NS_LOG_FUNCTION(this);
}
...
TypeId CtrlDMG_Poll::GetTypeId(void)
{
    NS_LOG_FUNCTION_NOARGS();
    static TypeId tid = TypeId("ns3::CtrlDMG_Poll")
	.SetParent<Header>()
	.AddConstructor<CtrlDMG_Poll>()
	;
    return tid;
}

TypeId CtrlDMG_Poll::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}

void CtrlDMG_Poll::Print(std::ostream &os) const
{
    NS_LOG_FUNCTION(this << &os);
    os << "Response Offset=" << m_responseOffset;
}

uint32_t CtrlDMG_Poll::GetSerializedSize() const
{
    NS_LOG_FUNCTION(this);
    return 2;  // Response Offset Field.
}

void CtrlDMG_Poll::Serialize(Buffer::Iterator start) const
{
    NS_LOG_FUNCTION (this << &start);
    Buffer::Iterator i = start;
...
uint32_t CtrlDMG_Poll::Deserialize(Buffer::Iterator start)
{
    NS_LOG_FUNCTION(this << &start);
    Buffer::Iterator i = start;
...
TypeId CtrlDMG_SPR::GetTypeId(void)
{
    NS_LOG_FUNCTION_NOARGS();
...
TypeId CtrlDMG_SPR::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}

void CtrlDMG_SPR::Print(std::ostream &os) const
{
    NS_LOG_FUNCTION(this << &os);
    m_dynamic.Print(os);
...
uint32_t CtrlDMG_SPR::GetSerializedSize() const
{
    NS_LOG_FUNCTION(this);
    return 7;  // Dynamic Allocation Info Field + BF Control.
}

void CtrlDMG_SPR::Serialize(Buffer::Iterator start) const
{
    NS_LOG_FUNCTION (this << &start);
    Buffer::Iterator i = start;

    m_dynamic.Serialize(i);
    m_bfControl.Serialize(i);
}

uint32_t CtrlDMG_SPR::Deserialize(Buffer::Iterator start)
{
    NS_LOG_FUNCTION(this << &start);
    Buffer::Iterator i = start;

    m_dynamic.Deserialize(i);
    m_bfControl.Deserialize(i);
...
TypeId CtrlDMG_Grant::GetTypeId(void)
{
    NS_LOG_FUNCTION_NOARGS();
...
TypeId CtrlDMG_Grant::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}
...
TypeId CtrlDMG_DTS::GetTypeId(void)
{
    NS_LOG_FUNCTION_NOARGS();
...
TypeId CtrlDMG_DTS::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}

void CtrlDMG_DTS::Print(std::ostream &os) const
{
    NS_LOG_FUNCTION(this << &os);

}

uint32_t CtrlDMG_DTS::GetSerializedSize() const
{
    NS_LOG_FUNCTION(this);
    return 12;  // NAV-SA + NAV-DA
}

void CtrlDMG_DTS::Serialize(Buffer::Iterator start) const
{
    NS_LOG_FUNCTION (this << &start);
    Buffer::Iterator i = start;
    uint8_t buffer[6];

    m_navSA.CopyTo(buffer);
    i.Write(buffer, 6);

    m_navDA.CopyTo(buffer);
    i.Write(buffer, 6);
}

uint32_t CtrlDMG_DTS::Deserialize(Buffer::Iterator start)
{
    NS_LOG_FUNCTION(this << &start);
    Buffer::Iterator i = start;
    uint8_t buffer[6];
    ...
    i.Read(buffer, 6);
    m_navSA.CopyFrom(buffer);

    i.Read(buffer, 6);
    m_navDA.CopyFrom(buffer);
...
TypeId CtrlDMG_SSW::GetTypeId(void)
{
    NS_LOG_FUNCTION_NOARGS();
...
TypeId CtrlDMG_SSW::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}
...
uint32_t CtrlDMG_SSW::GetSerializedSize() const
{
    NS_LOG_FUNCTION(this);
    return 6;  // SSW Field + SSW Feedback Field.
}

void CtrlDMG_SSW::Serialize(Buffer::Iterator start) const
{
    NS_LOG_FUNCTION (this << &start);
    Buffer::Iterator i = start;
    m_ssw.Serialize(i);
    m_sswFBCK.Serialize(i);
}

uint32_t CtrlDMG_SSW::Deserialize(Buffer::Iterator start)
{
    NS_LOG_FUNCTION(this << &start);
    Buffer::Iterator i = start;

    m_ssw.Deserialize(i);
    m_sswFBCK.Deserialize(i);
...
TypeId CtrlDMG_SSW_FBCK::GetTypeId(void)
{
    NS_LOG_FUNCTION_NOARGS();
...
TypeId CtrlDMG_SSW_FBCK::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}

void CtrlDMG_SSW_FBCK::Print(std::ostream &os) const
{
    NS_LOG_FUNCTION(this << &os);
    m_sswFBCK.Print(os);
    m_brp.Print(os);
...
uint32_t CtrlDMG_SSW_FBCK::GetSerializedSize() const
{
    NS_LOG_FUNCTION(this);
    return 8;  // SSW Feedback Field + BRP Request + Beamformed Link Maintenance.
}

void CtrlDMG_SSW_FBCK::Serialize(Buffer::Iterator start) const
{
    NS_LOG_FUNCTION (this << &start);
    Buffer::Iterator i = start;
    m_sswFBCK.Serialize(i);
    m_brp.Serialize(i);
    m_bfLink.Serialize(i);
}

uint32_t CtrlDMG_SSW_FBCK::Deserialize(Buffer::Iterator start)
{
    NS_LOG_FUNCTION(this << &start);
    Buffer::Iterator i = start;
...
TypeId CtrlDMG_SSW_ACK::GetTypeId(void)
{
    NS_LOG_FUNCTION_NOARGS();
...
TypeId CtrlDMG_SSW_ACK::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}
...
/*********************************************************
 *  Sector Sweep Feedback (SSW-Feedback) Frame (8.3.1.19)
 *********************************************************/

NS_OBJECT_ENSURE_REGISTERED (CtrlGrant_ACK);

CtrlGrant_ACK::CtrlGrant_ACK()
{
    NS_LOG_FUNCTION(this);
...
TypeId CtrlGrant_ACK::GetTypeId(void)
{
    NS_LOG_FUNCTION_NOARGS();
    static TypeId tid = TypeId("ns3::CtrlGrant_ACK")
	.SetParent<Header>()
	.AddConstructor<CtrlGrant_ACK>()
	;
    return tid;
}

TypeId CtrlGrant_ACK::GetInstanceTypeId(void) const
{
    NS_LOG_FUNCTION(this);
    return GetTypeId();
}

void CtrlGrant_ACK::Print(std::ostream &os) const
{
    NS_LOG_FUNCTION(this << &os);
    m_bfControl.Print(os);
}

uint32_t CtrlGrant_ACK::GetSerializedSize() const
{
    NS_LOG_FUNCTION(this);
    return 7;  // Reserved + BF Control.
}

void CtrlGrant_ACK::Serialize(Buffer::Iterator start) const
{
    NS_LOG_FUNCTION (this << &start);
    Buffer::Iterator i = start;
...
uint32_t CtrlGrant_ACK::Deserialize(Buffer::Iterator start)
{
    NS_LOG_FUNCTION(this << &start);
    Buffer::Iterator i = start;
