...
/**
 * Base class for DmgApWifiMac and DmgStaWifiMac.
 *
 * It is inspired by RegularWifiMac.
 *
 * It encapsulates all the common functionalities of DMG APs and STAs.
 *
 * In the current implementation it used a single EdcaTxOpN queue and holds a
 * reference to a DmgBeaconInterval.
 */
class DmgWifiMac : public WifiMac
{
public:
  ...
  void SetDmgAntennaController (Ptr<DmgAntennaController> dmgAntennaController);
  Ptr<DmgAntennaController> GetDmgAntennaController (void);
  void SetDmgBeaconInterval(Ptr<DmgBeaconInterval> dmgBi);
  Ptr<DmgBeaconInterval> GetDmgBeaconInterval(void);
  void SetPropagationGuard(Time guard);
  void StartDmgSpTracking (void);


  uint64_t GetBytesRxFrom (Ipv4Address src);
  /**
   * Set flow destin and start counting incoming packets for the destination with \param ipv4address.
   *
   */
  void InstallEnqueueRateMeter (Ipv4Address destin);
  /**
   * Start counting incoming packets for the destination with \param ipv4address.
   *
   */
  void ResetEnqueueRateMeter (Ipv4Address destin);

  /**
   * Stop counting packets and calculate the flow rate since last restart.
   *
   * \return the measured flow rate
   */
  uint32_t CalcEnqueueRate (Ipv4Address destin);
...
protected:
  ...
  Ptr<DmgAntennaController> m_dmgAntennaController; //!< Directional Antenna controller
  ...
  /** This holds a pointer to the Beamforming engine instance for this WifiMac - used
  for beamforming procedures with peer STAs. */
  // Ptr<BeamformingEngine> m_beamformingEngine;
 ...
 /**
  ...
  /**
   ...
   void SetDmgAckTimeoutGuard (Time guard);
   Time GetDmgAckTimeoutGuard (void) const;


private:
  DmgWifiMac (const DmgWifiMac &);
  DmgWifiMac & operator= (const DmgWifiMac &);
  ...
  Ptr<DmgBeaconInterval> m_dmgBeaconInterval;

  Time m_dmgAckTimeoutGuard;

  std::map <Ipv4Address, uint64_t> m_bytesRx; //bytes received since measurement starts 

  std::map <Ipv4Address, Time> m_startEnqueueMeasureT; //The time at which the flow rate measurement starts for the address
  std::map <Ipv4Address, uint64_t> m_bytesEnqueue; //bytes received since measurement starts 
};
