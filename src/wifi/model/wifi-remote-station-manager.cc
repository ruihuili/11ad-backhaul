...
void WifiRemoteStationManager::SetupPhy (Ptr<WifiPhy> phy)
{
    ...
    NS_LOG_DEBUG("DefaultTxMode = " << m_defaultTxMode.GetUniqueName());

    if(HasHtSupported())
    {
	m_defaultTxMcs = 0;//phy->GetMcs(0);
	NS_LOG_DEBUG("Has HT Supproted = True, DefaultTxMCS = " << m_defaultTxMcs);
    }
//    else if(HasHtSupported())
//    {
//	m_defaultTxMcs = phy->GetMcs(1);
//	NS_LOG_DEBUG("Has DMG Supproted = True, DefaultTxMCS = " << m_defaultTxMcs);
//    }
    else
    {
	m_defaultTxMcs = 0;
	NS_LOG_DEBUG("Has HT Supproted = False, DefaultTxMCS = 0");
    }
    Reset ();
}

void WifiRemoteStationManager::SetupMac (Ptr<WifiMac> mac)
{
...
void WifiRemoteStationManager::AddSupportedMcs (Mac48Address address, uint8_t mcs)
{
...
WifiMode WifiRemoteStationManager::GetControlAnswerMode(Mac48Address address, WifiMode reqMode)
{
  /**
...
WifiTxVector WifiRemoteStationManager::GetCtsTxVector(Mac48Address address, WifiMode rtsMode)
{
    NS_ASSERT (!address.IsGroup ());
    WifiTxVector v;
