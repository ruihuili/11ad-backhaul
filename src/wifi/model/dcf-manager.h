...
#include "ns3/dmg-beacon-interval.h"
#include <vector>
...
private:
  ...
  void NotifyDmgAccessGranted (void);
  void CancelDmgAccessGrantedFlag (void);
  /**
...
public:
  ...
  void SetIsDmg(bool isDmg);
  bool IsDmg(void);

  void SetNextSimpleDataExchangeDuration(Time dur);
...
private:
  ...
  /* Beacon Interval of this stations.
   * DcfManager use it for accessing the finromation about the start and stop
   * time of the Service Periods.
   */
  Ptr<DmgBeaconInterval> m_dmgBeaconInterval;
  /* If this is true, the DcfManager works in DMG mode.
   * Only transmissions in Service Periods are allowed.
   * CBAP not supported.
   */
  bool m_isDmg;
  /* duration of the next simple data exchange (data + ack).
   * Used by the DcfManager for checking if there is enough time for at least a
   * transmission in the current Service Period */
  Time m_nextSimpleDataExchangeDuration;
  /* Absolute start time of the next (or current) service period. */
  Time m_nextTxSpStart;
  /* Absolute stop time of the next (or current) service period. */
  Time m_nextTxSpStop;
  /* Role of the next (or current) service period. True if the station is the transmitter*/
  bool m_nextSpRole;
  /* True if the last transmission was successful */
  bool m_lastTxSuccess;
  /* Absolute end time of the last transmission (data + ack) */
  Time m_lastTxExchangeEnd;
  /* Estimated event for the next channel access grant */
  EventId m_dmgAccessTimeout;
  /* Propagation time (guard time) */
