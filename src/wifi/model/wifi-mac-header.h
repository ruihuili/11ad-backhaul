...
private:
  ...
  // New fields for 802.11ad
  uint16_t m_responseOffset;
  uint8_t m_dynamicAllocation[5];
  uint16_t m_bfControl;
  Mac48Address m_navSA;
  Mac48Address m_navDA;
  uint8_t m_ssw[3];
  uint8_t m_sswFBCK[3];
  uint32_t m_brpRequest;
  uint8_t m_beamformedLinkMaintenance;
  uint8_t m_reserved[5]; // For Grant ACK Frame

  // New Fields for DMG Support.
  uint32_t m_ctrlFrameExtension;

  // DMG Beacon fields.
  uint16_t m_dmg_duration;
  uint64_t m_dmg_bssid;
