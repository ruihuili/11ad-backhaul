...
public:
  ...
  Ptr<const Packet> GetNextPacketByAddress (WifiMacHeader &hdr, Mac48Address recipient);
  bool HasBar (struct Bar &bar);
  bool HasBarByAddress (struct Bar &bar, Mac48Address recipient);
  /**
  ...
  Ptr<const Packet> PeekNextPacketByAddress (WifiMacHeader &hdr, Mac48Address recipient, Time *timestamp);
  /**
  ...
  uint64_t GetMpduFailures (Mac48Address mac);
  uint64_t GetMpduSuccesses (Mac48Address mac);
...
private:
  ...
  std::map<Mac48Address, uint64_t> m_mpduFailuresPerDestination;
  std::map<Mac48Address, uint64_t> m_mpduSuccessesPerDestination;
  std::map<Mac48Address, uint64_t> m_mpduCounterPerDestination;
};
