...
public:
    ...
    * \param stbc enable or disable STBC (space-time block coding).
    */
    ...
    */
    ...
    /**
    * Sets if this PPDU is immediatly followed by another PPDU according to 9.13a
    *
    * \param value enable or disable APPDU.
    *
    * \param stbc enable or disable STBC
    */
    void SetAddPPDU(bool value);
    bool GetAddPPDU(void) const;

    void SetPacketType(enum PacketType type);
    enum PacketType GetPacketType(void) const;

    void SetTRN_Length(uint8_t length);
    uint8_t GetTRN_Length(void) const;

    void SetBeamTrackingRequest(bool value);
