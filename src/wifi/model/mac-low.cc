...
void
MacLowTransmissionParameters::SetCurrentSpEnd(Time end)
{
  m_currentSpEnd = end;
}

Time
MacLowTransmissionParameters::GetCurrentSpEnd(void) const
{
  return m_currentSpEnd;
}
...
MacLow::MacLow ()
  : m_normalAckTimeoutEvent (),
    ...
    m_dmgAckTimeoutGuard (MicroSeconds(1))
{
  ...
  m_macSampling = "00:00:00:00:00:00";
  m_seqSampling = 0;
}
...
bool MacLow::IsAmpdu(Ptr<const Packet> packet, const WifiMacHeader hdr)
{
...
bool MacLow::NeedCtsToSelf (void)
{
...
void MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiMode txMode, WifiPreamble preamble, bool ampduSubframe)
{
           ...
           && m_blockAckTimeoutEvent.IsRunning ())
    {
      ...
      m_ampdu = false;
        ...
        if (m_macSampling == hdr.GetAddr2 ()  && m_seqSampling == blockAck.GetStartingSequence())
        {
                ...
                NS_ASSERT(it_sampled != m_sampledDuration.end() && it_samplingStart != m_samplingStartTime.end());

                Time samp = Simulator::Now () - it_samplingStart->second;
...
WifiTxVector MacLow::GetDataTxVector(Ptr<const Packet> packet, const WifiMacHeader *hdr) const
{
    Mac48Address to = hdr->GetAddr1 ();
    WifiMacTrailer fcs;
...
void MacLow::ForwardDown(Ptr<const Packet> packet, const WifiMacHeader* hdr,
			 WifiTxVector txVector, WifiPreamble preamble)
{
	...
	NS_LOG_DEBUG("Sending " << queueSize << " MPDUS as part of A-MPDU " << Simulator::Now ()<< " from "<< m_currentHdr.GetAddr2()<<" to "<< m_currentHdr.GetAddr1());
	...
	for ( ; queueSize > 0; queueSize--)
	    ...
	    if (queueSize == 1) {
                last = true;
                packetType = 2; // Last packet in A-MPDU
           }
	    ...
	    if (delay == Seconds(0))
                {
                if (queueSize != 1) {
		      packetType = 1; // For all the MPDUs in an A-MPDU except the last one.
                }
	    ...
	    if(queueSize > 1)
		delay = delay + m_phy->CalculateTxDuration(GetSize(newPacket, &newHdr), txVector, preamble, m_phy->GetFrequency(), packetType, 0) + Seconds(1e-9);
	    preamble = WIFI_PREAMBLE_NONE;
        }
    }
}

void MacLow::SendPacket(Ptr<const Packet> packet, WifiTxVector txVector, WifiPreamble preamble, uint8_t packetType)
{
...
{
  NS_LOG_FUNCTION (this);
  NS_LOG_UNCOND (Simulator::Now ()<<"block ack timeout. from "<< m_currentHdr.GetAddr2()<<" to "<< m_currentHdr.GetAddr1());
  ...
  if (m_txParams.MustWaitNormalAck ())
    {
      Time timerDelay;
      if (m_isDmg) {
	// If we are in DMG mode compute the ideal ack timeout
	// (ack duration + 1us (guard_ + 2*propagation time)
        WifiMacHeader hdr;
        m_currentPacket->PeekHeader (hdr);
	timerDelay = txDuration + GetSifs() +
		GetAckDuration(hdr.GetAddr1(), dataTxVector) +
		m_dmgAckTimeoutGuard + m_propagationGuard * 2;
      } else {
  ...
  else if (m_txParams.MustWaitBasicBlockAck ())
    {
      ...
      if (m_isDmg) {
	timerDelay = txDuration + GetSifs() +
		GetBlockAckDuration(hdr.GetAddr1(), dataTxVector, BASIC_BLOCK_ACK) +
		m_dmgAckTimeoutGuard + m_propagationGuard * 2;
      } else {
  ...
  else if (m_txParams.MustWaitCompressedBlockAck ())
    {
      ...
      if (m_isDmg) {
	      timerDelay = txDuration + GetSifs() +
		    GetBlockAckDuration(hdr.GetAddr1(), dataTxVector, COMPRESSED_BLOCK_ACK) +
		    m_dmgAckTimeoutGuard + m_propagationGuard * 2;
      } else {
...
Time
MacLow::GetNextSimpleDataExchangeDuration (Ptr<const Packet> packet,
						const WifiMacHeader* hdr)
{
  WifiTxVector dataTxVector = GetDataTxVector(packet, hdr);
  ...
  Time txDuration = m_phy->CalculateTxDuration (size,
						dataTxVector, preamble,
						m_phy->GetFrequency(), 0, 0);
  Time ackDuration = GetAckDuration (hdr->GetAddr1(), dataTxVector);

  return (txDuration + GetSifs() + ackDuration);
}

Time
MacLow::GetNextSimpleAmpduExchangeDuration (Ptr<const Packet> packet,
						const WifiMacHeader* hdr)
{
  WifiTxVector dataTxVector = GetDataTxVector(packet, hdr);
  WifiPreamble preamble = WIFI_PREAMBLE_LONG;
  ...
  aggregated = m_mpduAggregator->Aggregate (packet, currentAggregatedPacket);

  if (!aggregated) {
    NS_FATAL_ERROR("Impossible to aggregate packet for computing SimpleAmpduExchangeDuration");
  }

  size = currentAggregatedPacket->GetSize () + hdr->GetSize () + fcs.GetSerializedSize ();

  Time txDuration = m_phy->CalculateTxDuration (size,
						dataTxVector, preamble,
						m_phy->GetFrequency(), 0, 0);
  Time blockAckDuration = GetBlockAckDuration(hdr->GetAddr1(), dataTxVector, COMPRESSED_BLOCK_ACK);

  return (txDuration + GetSifs() + blockAckDuration);
}
void MacLow::SendDataPacket (void)
{
	...
	if (m_txParams.HasNextPacket())
        {
	    duration += GetSifs ();
	    duration += m_phy->CalculateTxDuration(m_txParams.GetNextPacketSize (), dataTxVector, preamble, m_phy->GetFrequency(), 0, 0);
	    if (m_txParams.MustWaitAck())
            {
		duration += GetSifs ();
		duration += GetAckDuration (m_currentHdr.GetAddr1 (), dataTxVector);
	...
	m_currentPacket->AddTrailer (fcs);
        ...
        if (m_aggregateQueue->GetSize() == m_maxNumMpdu && m_currentPacket->GetSize() > 100)
        {
                ...
                std::map<Mac48Address, Time>::iterator it_sampledDuration = m_sampledDuration.find (m_currentHdr.GetAddr1());

                if(m_macSampling == "00:00:00:00:00:00"){
                        if (it_sampledDuration == m_sampledDuration.end())
                        {
                                //NS_LOG_UNCOND(m_self<< " Set sampling profile" << m_currentHdr.GetAddr1());

                                m_samplingStartTime.insert(std::make_pair(m_currentHdr.GetAddr1(), Seconds(0)));
                                m_sampledDuration.insert(std::make_pair(m_currentHdr.GetAddr1(), Seconds(0)));
                                m_sampleVarCounter.insert(std::make_pair(m_currentHdr.GetAddr1(), 0));
                        }

                        //start tracking
                        m_seqSampling = m_currentHdr.GetSequenceNumber();
                        m_macSampling = m_currentHdr.GetAddr1();

                        std::map<Mac48Address, Time>::iterator it_samplingStart = m_samplingStartTime.find(m_macSampling);

                        NS_ASSERT(it_samplingStart != m_samplingStartTime.end());

                        it_samplingStart->second = Simulator::Now ();    
                        //NS_LOG_UNCOND(Simulator::Now () <<" ["<< m_self<< "] Start sampling " << m_currentHdr.GetAddr1() << " seq " << m_seqSampling );
                }    
...
bool MacLow::ReceiveMpdu (Ptr<Packet> packet, WifiMacHeader hdr)
{
...
MacLow::RxCompleteBufferedPacketsWithSmallerSequence (uint16_t seq, Mac48Address originator, uint8_t tid)
{
  ...
  if (it != m_bAckAgreements.end ())
    {
      uint16_t endSequence = ((*it).second.first.GetStartingSequence () + 2047) % 4096;
      uint16_t mappedStart = QosUtilsMapSeqControlToUniqueInteger (seq << 4, endSequence);
...
void MacLow::SetMpduAggregator (Ptr<MpduAggregator> aggregator)
{
    m_mpduAggregator = aggregator;
}

void MacLow::DeaggregateAmpduAndReceive(Ptr<Packet> aggregatedPacket, double rxSnr, WifiMode txMode, WifiPreamble preamble)
{
    NS_LOG_FUNCTION(this << aggregatedPacket << rxSnr << txMode << preamble);
    m_currentMode = txMode;
    AmpduTag ampdu;
    bool normalAck = false;
    bool ampduSubframe = false;
    NS_LOG_DEBUG("Received aggregated packet size = " << aggregatedPacket->GetSize());
...
bool MacLow::StopAggregation(Ptr<const Packet> peekedPacket, WifiMacHeader peekedHdr, Ptr<Packet> aggregatedPacket, uint16_t size) const
{
    WifiPreamble preamble;
    WifiTxVector dataTxVector = GetDataTxVector(m_currentPacket, &m_currentHdr);

    if (m_phy->GetCurrentStandard() != WIFI_PHY_STANDARD_80211ad)
    {
	if (m_phy->GetGreenfield() && m_stationManager->GetGreenfieldSupported(m_currentHdr.GetAddr1()))
	    preamble = WIFI_PREAMBLE_HT_GF;
	else
    ...
    Time duration = m_phy->CalculateTxDuration(aggregatedPacket->GetSize() + peekedPacket->GetSize() +
					       peekedHdr.GetSize() + WIFI_MAC_FCS_LENGTH,dataTxVector, preamble, m_phy->GetFrequency(), 0, 0);

    if (m_phy->GetCurrentStandard() == WIFI_PHY_STANDARD_80211ad)
    {
	// An DMG STA shall not transmit a PPDU that has a duration that is greater than aPPDUMaxTime (2 milliseconds) Std 802.11ad-2012 Table 21-31.
...
bool MacLow::DmgStopAggregation(Ptr<const Packet> peekedPacket, WifiMacHeader peekedHdr, Ptr<Packet> aggregatedPacket, uint16_t size) const
{
  WifiPreamble preamble;
  WifiTxVector dataTxVector = GetDataTxVector(m_currentPacket, &m_currentHdr);
  ...
  Time blockAckDuration = GetBlockAckDuration(m_currentHdr.GetAddr1(), dataTxVector, COMPRESSED_BLOCK_ACK);

  if ((Simulator::Now() + duration + GetSifs() + blockAckDuration + 2 * m_propagationGuard + m_dmgAckTimeoutGuard) >=
      m_txParams.GetCurrentSpEnd()) {
    return true;
  }
...
Ptr<Packet> MacLow::AggregateToAmpdu (Ptr<const Packet> packet, const WifiMacHeader hdr)
{
    ...
    std::pair <Ipv4Address,Ipv4Address> srcSinkIpAddr;

    if(hdr.IsQosData())
      srcSinkIpAddr = InspectIpv4Addr(packet);
              ...
              if (peekedPacket == 0) 
                {
                  peekedPacket = queue->PeekByTidAndAddress (&peekedHdr, tid,
                                                             WifiMacHeader::ADDR1,
                                                             hdr.GetAddr1 (), srcSinkIpAddr, &tstamp);
               ...
               while (IsInWindow (currentSequenceNumber, startingSequenceNumber, 64) &&
                   !StopAggregation (peekedPacket, peekedHdr, currentAggregatedPacket, blockAckSize) &&
                   !DmgStopAggregation (peekedPacket, peekedHdr, currentAggregatedPacket, blockAckSize))
                {
                      ...
                      if (peekedPacket == 0)
                        {
                          ...
                          retry = false;
                          peekedPacket = queue->PeekByTidAndAddress (&peekedHdr, tid,
                                                                     WifiMacHeader::ADDR1, hdr.GetAddr1 (), srcSinkIpAddr, &tstamp);
                          if (peekedPacket != 0)
                            {
              ...
              if (isAmpdu || m_isDmg)
                {
                  if (i == 1 && hdr.IsQosData ())
                  {
                    ...
                    Ptr<const Packet> tmpPacket = queue->PeekByTidAndAddress (&tmpHdr, tid,
                                                             WifiMacHeader::ADDR1,
                                                             hdr.GetAddr1 (), srcSinkIpAddr, &tstamp);
                    listenerIt->second->CompleteMpduTx (packet, hdr, tstamp);
                  }
...
void MacLow::FlushAggregateQueue (void)
{
...
void
MacLow::SetDmgBeaconInterval (Ptr<DmgBeaconInterval> dmgBi)
{
  m_dmgBeaconInterval = dmgBi;
}

Ptr<DmgBeaconInterval>
MacLow::GetDmgBeaconInterval (void)
{
  return m_dmgBeaconInterval;
}

void
MacLow::SetIsDmg(bool isDmg)
{
  m_isDmg = isDmg;
}
...
void
MacLow::SetPropagationGuard (Time guard)
{
  m_propagationGuard = guard;
}

Time
MacLow::GetPropagationGuard (void)
{
  return m_propagationGuard;
}

void
MacLow::SetDmgAckTimeoutGuard (Time guard)
{
  m_dmgAckTimeoutGuard = guard;
}

Time
MacLow::GetDmgAckTimeoutGuard (void) const
{
  return m_dmgAckTimeoutGuard;
}

void
MacLow::CancelBlockAckTimeout (void)
{
  if (m_blockAckTimeoutEvent.IsRunning ())
    {
      m_blockAckTimeoutEvent.Cancel ();
...
MacLow::InspectIpv4Addr(Ptr<const Packet> packet)
{
        NS_LOG_FUNCTION(this);

        Ptr<Packet> packetCopy = packet->Copy ();
        //temp for debugging
        if(packetCopy->GetSize()!=1506)
        NS_LOG_DEBUG (Simulator::Now ()<<" "<< m_self << " (MACLOW )size " << packetCopy->GetSize());
        ...
        //temp for debugging
        if(packetCopy->GetSize()!=1478){
                NS_LOG_DEBUG (m_self << " size " << packetCopy->GetSize() << ". packet addressed from "<< srcsinkIp.first << "to " << srcsinkIp.second);
        }
...
Time 
MacLow::GetNMpduReturnDuration (Mac48Address mac)
{
