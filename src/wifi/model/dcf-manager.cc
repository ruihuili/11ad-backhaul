...
void DcfState::SetAifsn(uint32_t aifsn)
{
    m_aifsn = aifsn;
}

void DcfState::SetCwMin(uint32_t minCw)
{
    m_cwMin = minCw;
    ResetCw ();
}

void DcfState::SetCwMax(uint32_t maxCw)
{
    m_cwMax = maxCw;
    ResetCw ();
}

uint32_t DcfState::GetAifsn(void) const
{
    return m_aifsn;
}

uint32_t DcfState::GetCwMin(void) const
{
    return m_cwMin;
}

uint32_t DcfState::GetCwMax(void) const
{
    return m_cwMax;
}

void DcfState::ResetCw(void)
{
    m_cw = m_cwMin;
}

void DcfState::UpdateFailedCw (void)
{
    // see 802.11-2012, section 9.19.2.5
    m_cw = std::min(2 * (m_cw + 1) - 1, m_cwMax);
}

void DcfState::UpdateBackoffSlotsNow(uint32_t nSlots, Time backoffUpdateBound)
{
...
void DcfState::StartBackoffNow(uint32_t nSlots)
{
...
uint32_t DcfState::GetCw(void) const
{
    return m_cw;
}

uint32_t DcfState::GetBackoffSlots(void) const
{
    return m_backoffSlots;
}

Time DcfState::GetBackoffStart(void) const
{
    return m_backoffStart;
}

bool DcfState::IsAccessRequested(void) const
{
    return m_accessRequested;
}

void DcfState::NotifyAccessRequested(void)
{
    m_accessRequested = true;
}

void DcfState::NotifyAccessGranted(void)
{
...
void
DcfState::NotifyDmgAccessGranted (void)
{
    NS_ASSERT (m_accessRequested);
    DoNotifyAccessGranted ();
}
...
void
DcfState::CancelDmgAccessGrantedFlag (void)
{
    m_accessRequested = false;
}

void DcfState::NotifyCollision (void)
{
    DoNotifyCollision ();
}

void DcfState::NotifyInternalCollision (void)
{
    DoNotifyInternalCollision ();
}

void DcfState::NotifyChannelSwitching (void)
{
    DoNotifyChannelSwitching ();
}

void DcfState::NotifySleep (void)
{
    DoNotifySleep ();
}

void DcfState::NotifyWakeUp (void)
{
    DoNotifyWakeUp ();
}
...
DcfManager::DcfManager ()
  : m_lastAckTimeoutEnd (MicroSeconds (0)),
    ...
    m_isDmg (false),
    m_nextTxSpStart (NanoSeconds(0)),
    m_nextTxSpStop (NanoSeconds(0)),
    m_lastTxSuccess (false),
    m_lastTxExchangeEnd (NanoSeconds(0)),
    m_propagationGuard (NanoSeconds(0))
{
...
void DcfManager::SetupPhyListener(Ptr<WifiPhy> phy)
{
...
void DcfManager::RemovePhyListener(Ptr<WifiPhy> phy)
{
    NS_LOG_FUNCTION (this << phy);
    if (m_phyListener != 0)
    {
...
void DcfManager::SetupLowListener(Ptr<MacLow> low)
{
...
void DcfManager::SetSlot(Time slotTime)
{
  NS_LOG_FUNCTION (this << slotTime);
  m_slotTimeUs = slotTime.GetMicroSeconds ();
}

void DcfManager::SetSifs(Time sifs)
{
  NS_LOG_FUNCTION (this << sifs);
  m_sifs = sifs;
}

void DcfManager::SetEifsNoDifs(Time eifsNoDifs)
{
  NS_LOG_FUNCTION (this << eifsNoDifs);
  m_eifsNoDifs = eifsNoDifs;
}

Time DcfManager::GetEifsNoDifs() const
{
  NS_LOG_FUNCTION (this);
  return m_eifsNoDifs;
}

void DcfManager::Add(DcfState *dcf)
{
    NS_LOG_FUNCTION(this << dcf);
    m_states.push_back(dcf);
}

Time DcfManager::MostRecent(Time a, Time b) const
{
    NS_LOG_FUNCTION (this << a << b);
    return Max(a, b);
}

Time DcfManager::MostRecent(Time a, Time b, Time c) const
{
...
Time DcfManager::MostRecent(Time a, Time b, Time c, Time d) const
{
...
Time DcfManager::MostRecent(Time a, Time b, Time c, Time d, Time e, Time f) const
{
...
Time DcfManager::MostRecent (Time a, Time b, Time c, Time d, Time e, Time f, Time g) const
{
...
bool DcfManager::IsBusy(void) const
{
    NS_LOG_FUNCTION (this);
    NS_ASSERT(!m_isDmg);
    // PHY busy receiving.
...
void DcfManager::RequestAccess(DcfState *state)
{
    ...
    if (m_isDmg) {
      ...
      // Recover next (current) SP start and stop time
    if (now >= m_nextTxSpStop){
          //||(m_nextTxSpStop!= m_dmgBeaconInterval->GetNextTxSpStop())) {
	m_nextTxSpStart = m_dmgBeaconInterval->GetNextTxSpStart();
	m_nextTxSpStop = m_dmgBeaconInterval->GetNextTxSpStop();
      }

      NS_LOG_DEBUG(Simulator::Now() << " Request Access: SP start " << m_nextTxSpStart <<
		   " SP stop " << m_nextTxSpStop);
        
      //NS_ASSERT(m_nextTxSpStart <= now);

      // Compute the delay needed before trying to access the channel
      Time delay;
      if (now <= m_nextTxSpStart) { //We are not in our SP
	    delay = m_nextTxSpStart - now;
      } else { // We are inside our SP
	if (m_lastTxSuccess) { // Tx SIFS after the end of the last TX
	  if ((m_lastTxExchangeEnd + m_sifs) <= now) {
	    delay = NanoSeconds(0);
	  } else {
	    delay = (m_lastTxExchangeEnd + m_sifs) - now;
	  }
	} else { //TX immediately
	  if (now == m_lastAckTimeoutEnd) {
	    delay = NanoSeconds(1);
	  } else {
	    delay = NanoSeconds(0);
	  }
      ...
      if ((now + delay + m_nextSimpleDataExchangeDuration + m_propagationGuard * 2 ) < m_nextTxSpStop) { //If we are too late try to reschedule after dealy
	NS_LOG_DEBUG(Simulator::Now() << " Schedule AccessTimeout in " << delay << " (at " <<
		     now + delay << ")");
        m_dmgAccessTimeout = Simulator::Schedule (delay,
...
/*
void
DcfManager::RescheduleAccess ()
{
  if (m_isDmg) {
    Time now = Simulator::Now();
    NS_ASSERT(now >= m_nextTxSpStop);

    // Recover absolute start and stop time of the next (current) SP
    m_nextTxSpStart = m_dmgBeaconInterval->GetNextTxSpStart();
    m_nextTxSpStop = m_dmgBeaconInterval->GetNextTxSpStop();

    NS_LOG_DEBUG(Simulator::Now() << " Try to reschedule access: SP start " << m_nextTxSpStart <<
		   " SP stop " << m_nextTxSpStop);
    Time delay;

    // Compute the delay needed before trying to grant the channel
    if (now <= m_nextTxSpStart) { //We are not in our SP
      delay = m_nextTxSpStart - now;
    } else { // We are inside our SP
      if (m_lastTxSuccess) { // Tx SIFS after the end of the last TX
    ...
    NS_ASSERT((now + delay) < m_nextTxSpStop);

    NS_LOG_DEBUG(Simulator::Now() << " Schedule AccessTimeout in " << delay << " (at " <<
		     now + delay << ")");
    // Try to grant the channel after delay
    m_dmgAccessTimeout = Simulator::Schedule (delay,
					  &DcfManager::AccessTimeout, this);
...
void DcfManager::AccessTimeout(void)
{
    NS_LOG_FUNCTION (this);
    ...
    if (m_isDmg) {
      ...
      Time now = Simulator::Now();
        ...
        NS_ASSERT((now + m_nextSimpleDataExchangeDuration + m_propagationGuard * 2) <
                  m_nextTxSpStop);
        NS_LOG_DEBUG(Simulator::Now() << " Notify Channel Access Granted");
        state->NotifyDmgAccessGranted();

      /*if (m_nextTxSpStop !=m_dmgBeaconInterval->GetNextTxSpStop()) { //if schedule updated
        // Recover next (current) SP start and stop time
	m_nextTxSpStart = m_dmgBeaconInterval->GetNextTxSpStart();
	m_nextTxSpStop = m_dmgBeaconInterval->GetNextTxSpStop();
                if (now <= m_nextTxSpStart) {
                    Simulator::Schedule (m_nextTxSpStop - now, &DcfManager::RescheduleAccess, this);
                    NS_LOG_DEBUG(Simulator::Now() << "Schedule updated, try to reschedule in " << m_nextTxSpStop - now <<
		     " (at " << m_nextTxSpStop << ")");
                        return;
                }
      }

      NS_LOG_DEBUG(Simulator::Now() << " Next simple data exchange duration" <<
		     m_nextSimpleDataExchangeDuration); */
      // If we don't have enough time for a simple data exchange (data + ack)
      // try to reschedule in (m_nextTxSpStop - now
   /*   if ((now + m_nextSimpleDataExchangeDuration + m_propagationGuard * 2) >=
          m_nextTxSpStop) {
	Simulator::Schedule (m_nextTxSpStop - now,
				&DcfManager::RescheduleAccess, this);
	NS_LOG_DEBUG(Simulator::Now() << " Next simple data exchange end at " <<
		     now + m_nextSimpleDataExchangeDuration <<
		     " try to reschedule in " << m_nextTxSpStop - now <<
		     " (at " << m_nextTxSpStop << ")");
      } else { //else Grant channel access
	NS_LOG_DEBUG(Simulator::Now() << " Notify Channel Access Granted");
        state->NotifyDmgAccessGranted();
      }*/
...
Time DcfManager::GetAccessGrantStart(void) const
{
    NS_LOG_FUNCTION (this);
    NS_ASSERT(!m_isDmg);
    Time rxAccessStart;
...
Time DcfManager::GetBackoffStartFor(DcfState *state)
{
    NS_LOG_FUNCTION (this << state);
    Time mostRecentEvent = MostRecent(state->GetBackoffStart(), GetAccessGrantStart() + MicroSeconds(state->GetAifsn() * m_slotTimeUs));
    return mostRecentEvent;
}

Time DcfManager::GetBackoffEndFor(DcfState *state)
{
    return GetBackoffStartFor(state) + MicroSeconds(state->GetBackoffSlots() * m_slotTimeUs);
}

void DcfManager::UpdateBackoff(void)
{
    NS_LOG_FUNCTION (this);
    NS_ASSERT(!m_isDmg);
    uint32_t k = 0;
...
void DcfManager::DoRestartAccessTimeoutIfNeeded (void)
{
    NS_LOG_FUNCTION (this);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " Request to restart an Access Timeout will be ignored");
      return;
    }
    /**
    ...
    */
	...
	if (m_accessTimeout.IsRunning () && Simulator::GetDelayLeft (m_accessTimeout) > expectedBackoffDelay)
	{
	    m_accessTimeout.Cancel ();
	}
	if (m_accessTimeout.IsExpired ())
	{
	    m_accessTimeout = Simulator::Schedule (expectedBackoffDelay, &DcfManager::AccessTimeout, this);
	}
    }
}

void DcfManager::NotifyRxStartNow(Time duration)
{
    NS_LOG_FUNCTION (this << duration);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " Rx start " << duration);
      return;
    }
...
void DcfManager::NotifyRxEndOkNow(void)
{
    NS_LOG_FUNCTION (this);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " Rx end ok");
      return;
    }
    MY_DEBUG("rx end ok");
...
void DcfManager::NotifyRxEndErrorNow (void)
{
    NS_LOG_FUNCTION (this);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " Rx end error");
      return;
    }
...
void DcfManager::NotifyTxStartNow(Time duration)
{
    NS_LOG_FUNCTION (this << duration);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " Tx start " << duration);
      return;
    }
    if (m_rxing)
    {
...
void DcfManager::NotifyMaybeCcaBusyStartNow (Time duration)
{
    NS_LOG_FUNCTION (this << duration);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " Maybe CCA busy " << duration);
      return;
    }
...
void DcfManager::NotifySwitchingStartNow (Time duration)
{
  NS_LOG_FUNCTION (this << duration);
  NS_ASSERT(!m_isDmg);
  Time now = Simulator::Now ();
...
void DcfManager::NotifySleepNow (void)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT(!m_isDmg);
  m_sleeping = true;
...
void DcfManager::NotifyWakeupNow (void)
{
  NS_LOG_FUNCTION (this);
  NS_ASSERT(!m_isDmg);
  m_sleeping = false;
...
void DcfManager::NotifyNavResetNow (Time duration)
{
  NS_LOG_FUNCTION (this << duration);
...
void DcfManager::NotifyNavStartNow (Time duration)
{
    NS_LOG_FUNCTION (this << duration);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " NAV start " << duration);
      return;
    }
...
void DcfManager::NotifyAckTimeoutStartNow (Time duration)
{
    NS_LOG_FUNCTION (this << duration);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " Ack timeout start " << duration <<
		   " (end " << duration + Simulator::Now() << ")");
    }
    NS_ASSERT (m_lastAckTimeoutEnd < Simulator::Now ());
    m_lastAckTimeoutEnd = Simulator::Now () + duration;
}

void DcfManager::NotifyAckTimeoutResetNow ()
{
    NS_LOG_FUNCTION (this);
    if (m_isDmg) {
      NS_LOG_DEBUG(Simulator::Now() << " Ack timeout reset");
    }
    m_lastAckTimeoutEnd = Simulator::Now ();
    DoRestartAccessTimeoutIfNeeded ();
}

void DcfManager::NotifyCtsTimeoutStartNow (Time duration)
{
    NS_LOG_FUNCTION (this << duration);
    NS_ASSERT(!m_isDmg);
...
void DcfManager::NotifyCtsTimeoutResetNow ()
{
    NS_LOG_FUNCTION (this);
    NS_ASSERT(!m_isDmg);
...
void
DcfManager::SetDmgBeaconInterval (Ptr<DmgBeaconInterval> dmgBi)
{
  m_dmgBeaconInterval = dmgBi;
}

Ptr<DmgBeaconInterval>
DcfManager::GetDmgBeaconInterval (void)
{
  return m_dmgBeaconInterval;
}

void
DcfManager::SetIsDmg(bool isDmg)
{
  m_isDmg = isDmg;
}

bool
DcfManager::IsDmg(void)
{
  return m_isDmg;
}

void
DcfManager::SetNextSimpleDataExchangeDuration(Time dur)
{
  m_nextSimpleDataExchangeDuration = dur;
}
...
DcfManager::GotAck()
{
  DcfState *state = *(m_states.begin());
  if (m_isDmg) {
    NS_LOG_DEBUG(Simulator::Now() << " Got Ack");
  }
  m_lastTxExchangeEnd = Simulator::Now();
  m_lastTxSuccess = true;
  state->CancelDmgAccessGrantedFlag();
}

void
DcfManager::MissedAck()
{
  DcfState *state = *(m_states.begin());
  if (m_isDmg) {
    NS_LOG_DEBUG(Simulator::Now() << " Missed Ack");
  }
  m_lastTxExchangeEnd = Simulator::Now();
  m_lastTxSuccess = false;
  state->CancelDmgAccessGrantedFlag();
...
void
DcfManager::GotBlockAck()
{
  DcfState *state = *(m_states.begin());
  if (m_isDmg) {
    NS_LOG_DEBUG(Simulator::Now() << " Got BlockAck");
  }
  m_lastTxExchangeEnd = Simulator::Now();
  m_lastTxSuccess = true;
  state->CancelDmgAccessGrantedFlag();
...
void
DcfManager::MissedBlockAck()
{
  DcfState *state = *(m_states.begin());
  if (m_isDmg) {
    NS_LOG_DEBUG(Simulator::Now() << " Missed BlockAck");
  }
  m_lastTxExchangeEnd = Simulator::Now();
  m_lastTxSuccess = false;
    NS_LOG_DEBUG(" to CancelDmgAccessGrantedFlag ");
    state->CancelDmgAccessGrantedFlag();
    NS_LOG_DEBUG(" Canceled DmgAccessGrantedFlag ");
}

void
DcfManager::SetPropagationGuard (Time guard)
{
  m_propagationGuard = guard;
}

Time
DcfManager::GetPropagationGuard (void)
{
  return m_propagationGuard;
}
