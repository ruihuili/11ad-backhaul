...
public:
  ...
  */
  ...
  uint64_t GetFailures (Mac48Address mac);
  uint64_t GetSuccesses (Mac48Address mac);
  ...
  void ForceOriginatorBlockAckAgreement(Mac48Address dest, uint8_t tid,
       uint16_t timeout, bool immediateBAck);

  void StartDmgSpTracking (void);
  void SetMaxQueuePacketNumberPerDestination (uint32_t maxPacket);
...
private:
  ...
  std::map<Mac48Address, uint64_t> m_failuresPerDestination;
  std::map<Mac48Address, uint64_t> m_successesPerDestination;

  /* Beacon interval of this node */
  Ptr<DmgBeaconInterval> m_dmgBeaconInterval;
  /* if true the EdcaTxOpN works in DMG mode */
  bool m_isDmg;
  /* antenna controller of this node. 
   * Used to allign the antenna with the destination before transmitting
   */
  Ptr<DmgAntennaController> m_dmgAntennaController;
  Time m_nextSpStart;
  Time m_nextSpStop;
  EventId m_dmgSpStart;
  EventId m_dmgSpStop;
  Mac48Address m_dmgNextSpDestination;
  std::pair<Ipv4Address, Ipv4Address> m_dmgNextSpSrcSink;
};
