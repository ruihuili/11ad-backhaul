...
YansWifiPhy::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::YansWifiPhy")
    ...
    .AddAttribute ("ExtendedRateMode",
                   ...
                   MakeUintegerChecker<uint16_t> ())
    .AddAttribute ("dmgOfdm", "True for VHT OFDM, false for VHT SC",
                   BooleanValue (true),
                   MakeBooleanAccessor (&YansWifiPhy::GetDmgOfdm,
                                        &YansWifiPhy::SetDmgOfdm),
                   MakeBooleanChecker ())
  ;
  return tid;
}
...
enum WifiPhyStandard YansWifiPhy::GetCurrentStandard(void) const
{
    return m_standard;
}
        ...
        case YansWifiPhy::IDLE:
            if (rxPowerW > m_edThresholdW)
            {
                if (IsModeSupported(txMode) || IsMcsSupported(txMode))
                {
                    ...
                    NS_LOG_DEBUG ("sync to signal (power=" << rxPowerW << "W). Time now: "<<Simulator::Now());
                    // sync to signal
            ...
            {
                NS_LOG_DEBUG("drop packet because signal power too Small (" << rxPowerW << "<" << m_edThresholdW << ")");
                NotifyRxDrop (packet);
                goto maybeCcaBusy;
            }
            break;
...
void YansWifiPhy::SendPacket(Ptr<const Packet> packet, WifiTxVector txVector, WifiPreamble preamble, uint8_t packetType)
{
    ...
    *  - we are idle
    */
    NS_LOG_INFO ("m_state->IsStateTx ()"<< m_state->IsStateTx () << " !m_state->IsStateSwitching () " << !m_state->IsStateSwitching ());
    ...
    NS_LOG_DEBUG("PHY Packet Duration = " << txDuration<< ". Time now: "<< Simulator::Now());
...
void YansWifiPhy::Configure80211ad (void)
{
  NS_LOG_FUNCTION (this);
  m_channelStartingFrequency = 59.4e3; // 59.4 GHz

  m_deviceRateSet.push_back (WifiPhy::GetVHTMCS0 ());
...
void
YansWifiPhy::SetDmgAntennaController (Ptr<DmgAntennaController> dmgAntennaController)
{
  m_dmgAntennaController = dmgAntennaController;
}

Ptr<DmgAntennaController>
YansWifiPhy::GetDmgAntennaController (void)
{
...
YansWifiPhy::EndReceive(Ptr<Packet> packet, Ptr<InterferenceHelper::Event> event)
{
    ...
    ", snr=" << snrPer.snr << ", per=" << snrPer.per << ", size=" << packet->GetSize()<< ". Time now: "<< Simulator::Now());

    double minPer = 1 - std::pow(1 - m_frameMinBer, packet->GetSize() * 8);
    if (snrPer.per < minPer) {
      snrPer.per = minPer;
    }
    ...
    {
        NS_LOG_DEBUG("m_random->GetValue() < snrPer.per, rx drop");
        /* Failure. */
        NotifyRxDrop(packet);
...
{
    NS_LOG_DEBUG ("McsToWifiMode " << mcs);
    WifiMode mode;
    switch (mcs)
     { 
       case 7:
...
void
YansWifiPhy::SetFrameMinBer (double per)
{
  m_frameMinBer = per;
}

double
YansWifiPhy::GetFrameMinBer (void)
{
  return m_frameMinBer;
}

void
YansWifiPhy::SetAddress (Mac48Address ad)
{
  m_self = ad;
}

Mac48Address
YansWifiPhy::GetAddress (void)
{
  return m_self;
}

void
YansWifiPhy::SetDmgOfdm(bool dmgOfdm)
{
  m_dmgOfdm = dmgOfdm;
}

bool
YansWifiPhy::GetDmgOfdm(void) const
{
  return m_dmgOfdm;
}
