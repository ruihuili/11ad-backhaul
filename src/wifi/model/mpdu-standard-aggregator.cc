...
TypeId MpduStandardAggregator::GetTypeId (void)
{
...
bool MpduStandardAggregator::Aggregate(Ptr<const Packet> packet, Ptr<Packet> aggregatedPacket)
{
...
void MpduStandardAggregator::AddHeaderAndPad (Ptr<Packet> packet, bool last)
{
    NS_LOG_FUNCTION (this);
    AmpduSubframeHeader currentHdr;
...
bool MpduStandardAggregator::CanBeAggregated (uint32_t packetSize, Ptr<Packet> aggregatedPacket, uint8_t blockAckSize)
{
    uint32_t padding = CalculatePadding (aggregatedPacket);
    uint32_t actualSize = aggregatedPacket->GetSize ();
    if (blockAckSize > 0)
...
uint32_t MpduStandardAggregator::CalculatePadding (Ptr<const Packet> packet)
{
