...
WifiPhy::WifiPhy ()
{
    ...
    m_totalAmpduSizeDmg = 0;
    m_totalAmpduNumSymbolsDmg = 0;
}
...
Time WifiPhy::GetPlcpHtTrainingSymbolDuration (WifiPreamble preamble, WifiTxVector txvector)
{
    uint8_t Ndltf, Neltf;
...
//return L-SIG
Time WifiPhy::GetPlcpHtSigHeaderDuration(WifiMode payloadMode, WifiPreamble preamble)
{
    switch (preamble)
    {
        case WIFI_PREAMBLE_HT_MF:
...
WifiMode WifiPhy::GetPlcpHeaderMode(WifiMode payloadMode, WifiPreamble preamble)
{
	...
	case WIFI_MOD_CLASS_VHT_CTRL:
	    return WifiPhy::GetVHTMCS0();

        case WIFI_MOD_CLASS_VHT_SC:
            return WifiPhy::GetVHTMCS1();

        case WIFI_MOD_CLASS_VHT_OFDM:
            return WifiPhy::GetVHTMCS13a();
...
Time WifiPhy::GetPlcpPreambleDuration(WifiMode payloadMode, WifiPreamble preamble)
{
	...
	case WIFI_MOD_CLASS_VHT_OFDM:
	    ...
	    // OFDM Preamble = 1890 micro seconds.
	    return NanoSeconds(1891);
...
Time WifiPhy::GetPlcpHeaderDuration(WifiMode payloadMode, WifiPreamble preamble)
{
    if (preamble == WIFI_PREAMBLE_NONE)
    {
	return MicroSeconds(0);
...
Time WifiPhy::GetPayloadDuration(uint32_t size, WifiTxVector txvector, WifiPreamble preamble, double frequency, uint8_t packetType, uint8_t incFlag)
{
	...
	case WIFI_MOD_CLASS_HT:
	{
	    ...
	    if (frequency >= 2400 && frequency <= 2500 && ((packetType == 0 && preamble != WIFI_PREAMBLE_NONE) ||
							   (packetType == 2 && preamble == WIFI_PREAMBLE_NONE))) //at 2.4 GHz
	    {
		return Time (numSymbols * symbolDuration) + MicroSeconds(6);
	    }
	    else  //at 5 GHz
    ...
    //TODO: This code must be checked
	    double Ncw;		    /* Number of LDPC codewords. */
	    double Ldpcw;		    /* Number of bits in the second and any subsequent codeword except the last. */
	    double Ldplcw;		    /* Number of bits in the last codeword. */
	    double DencodedSymmbols;	    /* Number of differentailly encoded payload symbols. */
	    uint32_t Nbits = (size * 8);    /* Nbits = Number of bits in the payload part. */


      if (packetType == 1 && preamble != WIFI_PREAMBLE_NONE) {
	      Ncw = 1 + ((size - 6) * 8/168.0);
	      Ldpcw =  ((size - 6) * 8.0/(Ncw - 1));
	      Ldplcw = (size - 6) * 8 - (Ncw - 2) * Ldpcw;
	      DencodedSymmbols = (Ldpcw + 168) * (Ncw - 2) + (Ldplcw + 168);
        DencodedSymmbols = DencodedSymmbols * 32; // spreading

        if (incFlag == 1) {
		      m_totalAmpduSizeDmg = size;
		      m_totalAmpduNumSymbolsDmg = DencodedSymmbols;
		    }
      } else if (packetType == 1 && preamble == WIFI_PREAMBLE_NONE) {
	      Ncw = 1 + ((size - 6) * 8/168.0);
	      Ldpcw =  ((size - 6) * 8.0/(Ncw - 1));
        ...
        if (incFlag == 1) {
		    ...
		    uint32_t totalAmpduSize = m_totalAmpduSizeDmg + size;

	      Ncw = 1 + ceil((totalAmpduSize - 6) * 8/168.0);
	      Ldpcw =  ceil((totalAmpduSize - 6) * 8.0/(Ncw - 1));
	      Ldplcw = (totalAmpduSize - 6) * 8 - (Ncw - 2) * Ldpcw;
	      DencodedSymmbols = (Ldpcw + 168) * (Ncw - 2) + (Ldplcw + 168);
        DencodedSymmbols = DencodedSymmbols * 32; // spreading

		    NS_ASSERT (m_totalAmpduNumSymbolsDmg <= DencodedSymmbols);
		    DencodedSymmbols -= m_totalAmpduNumSymbolsDmg;
		    if (incFlag == 1) {
		      m_totalAmpduSizeDmg = 0;
		      m_totalAmpduNumSymbolsDmg = 0;
		    }
      } else {
	      Ncw = 1 + ceil((size - 6) * 8/168.0);
	      //Ldpcw = ((Ncw > 1) ? (uint32_t) ceil((size - 6) * 8/(Ncw - 1)) : 0);
	      Ldpcw =  ceil((size - 6) * 8.0/(Ncw - 1));
	      //Ldplcw = ((Ncw > 1) ? (size - 6) * 8 - (Ncw - 2) * Ldpcw : 0);
	      Ldplcw = (size - 6) * 8 - (Ncw - 2) * Ldpcw;
	      DencodedSymmbols = (Ldpcw + 168) * (Ncw - 2) + (Ldplcw + 168);
        DencodedSymmbols = DencodedSymmbols * 32; // spreading
      }
	...
	case WIFI_MOD_CLASS_VHT_SC:
	{
      ...
      // TODO: This code must be checked
	    ...
	    uint32_t Ncbps; // Ncbpb = Number of coded bits per symbol block. Check Table 21-20 for different constellations.
	    ...
	    double Nbits = (size * 8); /* Nbits = Number of bits in the payload part. */
	    double Ncbits;             /* Ncbits = Number of coded bits in the payload part. */
	    double Ncw;
	    double Nblks;
        ...
        if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_1_4)
	      ...
	      Ncw = (Ncbits / 672.0);         /* Ncw = The number of LDPC codewords.  */
	      Nblks = (Ncw * 672.0 / Ncbps);   /* Nblks = Number of OFDM Symbols. */

        if (incFlag == 1) {
		      m_totalAmpduSizeDmg = size;
		      m_totalAmpduNumSymbolsDmg = Nblks;
		    }
      } else if (packetType == 1 && preamble == WIFI_PREAMBLE_NONE) {
	      if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_1_4)
		      Ncbits = Nbits * 4;
        ...
        if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_1_4)
		      ...
		      Ncbits = ceil(Nbits * 16.0 / 13);
	      else if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_3_4)
		      Ncbits = ceil(Nbits * 4.0 / 3);
	      else if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_5_8)
		      Ncbits = ceil(Nbits * 8.0 / 5);
	      else
          NS_FATAL_ERROR("unsupported code rate");

	      Ncw = ceil(Ncbits / 672.0);         /* Ncw = The number of LDPC codewords.  */
	      Nblks = ceil(Ncw * 672.0 / Ncbps);   /* Nblks = Number of OFDM Symbols. */


		    NS_ASSERT (m_totalAmpduNumSymbolsDmg <= Nblks);
		    Nblks -= m_totalAmpduNumSymbolsDmg;
		    if (incFlag == 1) {
		      m_totalAmpduSizeDmg = 0;
		      m_totalAmpduNumSymbolsDmg = 0;
		    }
	    ...
	    /* Make sure the result is in nanoseconds. */
	    double ret = ((Nblks * 512 + 64) * 1.0e3)/ 1760.0;
	    NS_LOG_DEBUG("bits " << Nbits << " cbits " << Ncbits << " rate " << payloadMode.GetDataRate() << " Payload Time " << ret << " ns");
    ...
    //TODO: This code must be checked
        ...
        if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_1_4)
	      ...
	      //Ncw = ceil(Ncbits / 672.0);         /* Ncw = The number of LDPC codewords.  */
	      //Nsym = ceil(Ncw * 672.0 / Ncbps);   /* Nsym = Number of OFDM Symbols. */
	      Ncw = (Ncbits / 672.0);         /* Ncw = The number of LDPC codewords.  */
	      Nsym = (Ncw * 672.0 / Ncbps);   /* Nsym = Number of OFDM Symbols. */

        if (incFlag == 1) {
		      m_totalAmpduSizeDmg = size;
		      m_totalAmpduNumSymbolsDmg = Nsym;
		    }
      } else if (packetType == 1 && preamble == WIFI_PREAMBLE_NONE) {
	      if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_1_4)
		      Ncbits = Nbits * 4;
        ...
        if (payloadMode.GetCodeRate() == WIFI_CODE_RATE_1_4)
	      ...
	      Nsym = ceil(Ncw * 672.0 / Ncbps);   /* Nsym = Number of OFDM Symbols. */
		    ...
		    NS_ASSERT (m_totalAmpduNumSymbolsDmg <= Nsym);
		    Nsym -= m_totalAmpduNumSymbolsDmg;
		    if (incFlag == 1) {
		      m_totalAmpduSizeDmg = 0;
		      m_totalAmpduNumSymbolsDmg = 0;
		    }
	    ...
	    /* Make sure the result is in nanoseconds */
	    double ret = Nsym * 242; /* Tsys(OFDM) = 242ns */
	    NS_LOG_DEBUG("bits " << Nbits << " cbits " << Ncbits << " rate " << payloadMode.GetDataRate() <<
          " Ncw " << Ncw << " Nsym " << Nsym << " Payload Time " << ret << " ns " <<
          " m_totalAmpduSize " << m_totalAmpduSizeDmg << " m_totalAmpduNumSymbols " << m_totalAmpduNumSymbolsDmg);
...
WifiPhy::CalculateTxDuration(uint32_t size, WifiTxVector txvector, WifiPreamble preamble, double frequency, uint8_t packetType, uint8_t incFlag)
{
   ...
   // NS_LOG_UNCOND (Simulator::Now () << " GetPlcpPreambleDuration(payloadMode, preamble) " << GetPlcpPreambleDuration(payloadMode, preamble) << " GetPlcpHeaderDuration " <<GetPlcpHeaderDuration(payloadMode, preamble)<< " GetPlcpHtTrainingSymbolDuration " << GetPlcpHtTrainingSymbolDuration(preamble, txvector)<<" GetPayloadDuration "<<GetPayloadDuration(size, txvector, preamble, frequency, packetType, incFlag)<<" size "<<size);
    return duration;
}
...
/* DMG Control PHY */
WifiMode WifiPhy::GetVHTMCS0()
{
    static WifiMode mode =
	   WifiModeFactory::CreateWifiMode("VHTMCS0",
					   WIFI_MOD_CLASS_VHT_CTRL,
