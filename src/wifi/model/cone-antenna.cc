  ...
  static TypeId tid = TypeId ("ns3::ConeAntenna")
    ...
    .AddAttribute ("Elevation",
			   ...
			   &ConeAntenna::GetElevationAngle),
                   MakeDoubleChecker<double> ())
    .AddAttribute ("GainDbiOutsideBeam",
                   "TX and RX gain in Dbi outside the beam",
                   DoubleValue (-1000000),
                   MakeDoubleAccessor (&ConeAntenna::SetGainDbiOutsideBeam,
			   &ConeAntenna::GetGainDbiOutsideBeam),
                   MakeDoubleChecker<double> ())
    ;
  return tid;
}
...
ConeAntenna::ConeAntenna()
	: m_gainDbi(0), m_beamwidth(2*M_PI), m_azimuth(0), m_elevation(0)
{
    NS_LOG_FUNCTION("");
}
...
double ConeAntenna::GetTxGainDbi(double azimuth, double elevation) const
{
    NS_LOG_FUNCTION(azimuth << elevation);
    if ((getAngleDiff(azimuth, m_azimuth) > m_beamwidth/2) ||
	(getAngleDiff(elevation, m_elevation) > m_beamwidth/2))
            //return -100;
            return m_gainDbiOutsideBeam;
    return m_gainDbi;
}

double ConeAntenna::GetRxGainDbi(double azimuth, double elevation) const
{
    NS_LOG_FUNCTION(azimuth << elevation);
    if ((getAngleDiff(azimuth,m_azimuth) > m_beamwidth/2) ||
        (getAngleDiff(elevation,m_elevation) > m_beamwidth/2))
...
ConeAntenna::SetGainDbi(double gainDbi)
{
    ...
    NS_LOG_FUNCTION("gainDbi" << m_gainDbi <<
                    " beamwidth" << m_beamwidth);
}

double
ConeAntenna::GetGainDbiOutsideBeam (void) const
{
  return m_gainDbiOutsideBeam;
}

void
ConeAntenna::SetGainDbiOutsideBeam (double gainDbi)
{
  m_gainDbiOutsideBeam = gainDbi;
}
