...
public:
  ...
  Ptr<const Packet> PeekByTidAndAddress (WifiMacHeader *hdr,
    ...
    Ptr<const Packet> DequeueByAddress (WifiMacHeader *hdr,
                                              WifiMacHeader::AddressType type,
                                              Mac48Address addr,
                                            std::pair <Ipv4Address,Ipv4Address> srcsinkIp);
    
    Ptr<const Packet> PeekByAddress (WifiMacHeader *hdr,
                                           WifiMacHeader::AddressType type,
                                           Mac48Address addr,
                                           std::pair <Ipv4Address,Ipv4Address> srcsinkIp,
                                           Time *timestamp);
  ...
  /**
   * Double the queue size when queue is about to overflow.
   * Half the queue size when the queue is short for a certain time
   * 
   */
  void DynamicQueueSize (void);
...
protected:
   ...
   * \return the address
   */
  Mac48Address GetNextHopMacAddressForPacket (enum WifiMacHeader::AddressType type, PacketQueueI it);

  /**
   * Return the Ipv4 address for the given packet (given by PacketQueue iterator).
   *
   * \param type
   * \param it
   * \return the address
   */
  std::pair <Ipv4Address,Ipv4Address> GetSrcDestinIpv4AddressForPacket (PacketQueueI it);
  ...
  bool m_shallowBufTimerOn; // if the shallow buffer has been recorded 
  Time m_shallowBufStart;   // The time at which the buffer has been reportedly shallow 
  Time m_shallowBufDurThresh;   // The duration to wait after discovering the buffer has been shallow and before actually shrinking the buffer size 
};
