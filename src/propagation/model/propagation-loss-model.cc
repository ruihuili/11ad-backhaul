...
// ----------------------60GHz DMG pathloss model-------Maybe move to wifi---------------------//
    
    NS_OBJECT_ENSURE_REGISTERED (FriisLoSPropagationLossModel);
    
    TypeId
    FriisLoSPropagationLossModel::GetTypeId (void)
    {
        static TypeId tid = TypeId ("ns3::FriisLoSPropagationLossModel")
        .SetParent<PropagationLossModel> ()
        .AddConstructor<FriisLoSPropagationLossModel> ()
        .AddAttribute ("Frequency",
                       "The carrier frequency (in Hz) at which propagation occurs  (default is 5.15 GHz).",
                       DoubleValue (5.150e9),
                       MakeDoubleAccessor (&FriisLoSPropagationLossModel::SetFrequency,
                                           &FriisLoSPropagationLossModel::GetFrequency),
                       MakeDoubleChecker<double> ())
        .AddAttribute ("SystemLoss", "The system loss",
                       DoubleValue (1.0),
                       MakeDoubleAccessor (&FriisLoSPropagationLossModel::m_systemLoss),
                       MakeDoubleChecker<double> ())
        .AddAttribute ("MinLoss",
                       "The minimum value (dB) of the total loss, used at short ranges. Note: ",
                       DoubleValue (0.0),
                       MakeDoubleAccessor (&FriisLoSPropagationLossModel::SetMinLoss,
                                           &FriisLoSPropagationLossModel::GetMinLoss),
                       MakeDoubleChecker<double> ())
        ;
        return tid;
    }
    
    FriisLoSPropagationLossModel::FriisLoSPropagationLossModel ()
    {
    }
    void
    FriisLoSPropagationLossModel::SetSystemLoss (double systemLoss)
    {
        m_systemLoss = systemLoss;
    }
    double
    FriisLoSPropagationLossModel::GetSystemLoss (void) const
    {
        return m_systemLoss;
    }
    void
    FriisLoSPropagationLossModel::SetMinLoss (double minLoss)
    {
        m_minLoss = minLoss;
    }
    double
    FriisLoSPropagationLossModel::GetMinLoss (void) const
    {
        return m_minLoss;
    }
    
    void
    FriisLoSPropagationLossModel::SetFrequency (double frequency)
    {
        m_frequency = frequency;
        static const double C = 299792458.0; // speed of light in vacuum
        m_lambda = C / frequency;
    ...
    double
    FriisLoSPropagationLossModel::GetFrequency (void) const
    {
        return m_frequency;
    }
    
    double
    FriisLoSPropagationLossModel::DbmToW (double dbm) const
    {
        double mw = std::pow (10.0,dbm/10.0);
        return mw / 1000.0;
    }
    ...
    double
    FriisLoSPropagationLossModel::DbmFromW (double w) const
    {
        double dbm = std::log10 (w * 1000.0) * 10.0;
        return dbm;
    }
    ...
    double
    FriisLoSPropagationLossModel::DoCalcRxPower (double txPowerDbm,
                                                 Ptr<MobilityModel> a,
                                                 Ptr<MobilityModel> b) const
    {
        double distance = a->GetDistanceFrom (b);
        ...
        std::map<MobilityPair, bool>::const_iterator i = m_los.find (std::make_pair (a, b));
    ...
    int64_t
    FriisLoSPropagationLossModel::DoAssignStreams (int64_t stream)
    {
        return 0;
    }
    
    void
    FriisLoSPropagationLossModel::SetLoS (Ptr<MobilityModel> ma, Ptr<MobilityModel> mb, bool los)
    {
        NS_ASSERT (ma != 0 && mb != 0);
        
        MobilityPair p = std::make_pair (ma, mb);
        std::map<MobilityPair, bool>::iterator i = m_los.find (p);
        
        if (i == m_los.end ())
        {
            m_los.insert (std::make_pair (p, los));
        }
        else
        {
            i->second = los;
