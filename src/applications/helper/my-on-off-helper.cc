  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
      ...
      for (uint32_t j = 0; j < node->GetNApplications (); j++)
        {
          Ptr<MyOnOffApplication> myonoff = DynamicCast<MyOnOffApplication> (node->GetApplication (j));
          if (myonoff)
            {
              currentStream += myonoff->AssignStreams (currentStream);
            }
        }
    }
...
void 
MyOnOffHelper::SetVariableRate (uint32_t packetSize)
{
  ...
  m_factory.Set ("Interval", StringValue ("ns3::ConstantRandomVariable[Constant=14700]"));
