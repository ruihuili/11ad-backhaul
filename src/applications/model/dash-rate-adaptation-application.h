...
#ifndef DASH_RATE_ADAPTATION_APPLICATION_H
#define DASH_RATE_ADAPTATION_APPLICATION_H
...
*/
class DashRateAdaptationApplication : public Application
{
public:
  /**
  ...
  DashRateAdaptationApplication ();

  virtual ~DashRateAdaptationApplication();
  ...
  */
  ...
  /*
  * Set bit rate map from main script
  */
  void SetBitRateMap (std::map <uint32_t, double> prob);

  uint64_t GetTotalTxBytes (void);
...
private:
  ...
  Ptr<RandomVariableStream>  m_vbrInterval;
  Ptr<RandomVariableStream>  m_vbrIntervalFailSafe;
  Ptr<RandomVariableStream>  m_vbrRateProbInterval;
  ...
  uint32_t        m_sent;
  std::map<uint32_t, double> m_rateMap; //different from the above attributes, m_rateMap is set via SetRateMap(), currently
...
private:
  ...
  /* map from probability interval, in which the randomly generated number falls into, 
  *to the bit rate, using map
  */
  uint32_t MapBitRate ();
};
...
#endif /* DASH_RATE_ADAPTATION_APPLICATION_H */
