...
int main (int argc, char *argv[])
{
	...
	double ackTraffFrac = 0.057039;//0.06;//94/(1554+94)= 0.057038835
	...
	else if(config.scenario <= 10)
	{
		if(config.trafficType == "udp")
			config.dir_oss << "topoNottin/scenario"  << config.scenario << "/udp30s/";
		else if (config.trafficType == "tcp")		
			config.dir_oss << "topoNottin/scenario"  << config.scenario << "/tcpseed"<< config.rngSeed <<"/" ;
		NS_LOG_INFO("output path: " << config.dir_oss.str());
	}
